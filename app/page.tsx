...
const templatePositions = [
  [25, 236], [145, 236], [266, 236], [25, 369], [145, 369], [266, 369], [25, 502], [145, 502], [266, 502], [145, 635],
];
...
export default function ToSignSite() {
  ...
  useEffect(() => {
    ...
    base.onload = async () => {
      ...
      ctx.font = `${toFont}px HYShiGuangTiW, KaiTi, STKaiti, serif`; ctx.fillText(`To：${name.trim()}`, padding, padding);
      let lines: string[] = []; let lineHeight = 0; do { ctx.font = `${copyFont}px HYShiGuangTiW, KaiTi, STKaiti, serif`; lines = wrap(ctx, copy, usableWidth); lineHeight = copyFont * 1.26; if (copyTop + lines.length * lineHeight <= boxHeight - padding) break; copyFont -= 1; } while (copyFont > 14);
      ctx.font = `${copyFont}px HYShiGuangTiW, KaiTi, STKaiti, serif`; lines.forEach((line, index) => ctx.fillText(line, padding, copyTop + index * lineHeight)); ctx.restore();
    };
...
function GalleryMenu({ onBack, onOpen }: { onBack: () => void; onOpen: (kind: keyof typeof galleryGroups) => void }) { return <div className="artboard dots gallery-motion"><Back onClick={onBack} /><div className="folder-carousel" onWheel={horizontalWheel}>{(Object.keys(galleryGroups) as (keyof typeof galleryGroups)[]).map((kind) => <button key={kind} onClick={() => onOpen(kind)} aria-label={`打开 ${kind} 图片文件夹`}><img src={`/assets/${folderFor(kind)}/${galleryGroups[kind][0]}`} alt="" /><b>{kind}</b><span>点击查看</span></button>)}</div></div>; }

function Gallery({ kind, onBack, onOpen }: { kind: keyof typeof galleryGroups; onBack: () => void; onOpen: (src: string) => void }) { const folder = folderFor(kind); return <div className="artboard dots gallery-art"><Back onClick={onBack} /><h1>{kind}</h1><p>点击图片即可查看高清图</p><div className="image-grid">{galleryGroups[kind].map((file) => { const src = `/assets/${folder}/${file}`; return <button onClick={() => onOpen(src)} key={file}><img src={src} alt="君君的插画" /></button>; })}</div></div>; }
